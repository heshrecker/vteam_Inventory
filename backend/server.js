...
// Create tables if not exist, including printedUser in goodsOut
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS goodsOut (
    ...
    printedUser TEXT -- NEW COLUMN for who printed
  );
...
app.post('/goodsOut', (req, res) => {
  const { receiver, items, printedUser } = req.body;
  if (!receiver || !Array.isArray(items) || !printedUser) {
    return res.status(400).json({ error: 'Invalid data' });
  }
  try {
    const stmt = db.prepare(`
      INSERT INTO goodsOut (date, receiver, items, printedUser)
      VALUES (?, ?, ?, ?)
    `);
    stmt.run(new Date().toISOString(), receiver, JSON.stringify(items), printedUser);
    res.sendStatus(200);
  } catch (error) {
...
// DELETE goods out record by id
app.delete('/goodsOut/:id', (req, res) => {
  const id = Number(req.params.id);
  if (!id) return res.status(400).json({ error: 'Invalid ID' });

  try {
    const record = db.prepare('SELECT * FROM goodsOut WHERE id = ?').get(id);
    if (!record) {
      return res.status(404).json({ error: 'Record not found' });
    }

    const del = db.prepare('DELETE FROM goodsOut WHERE id = ?');
    del.run(id);

    res.status(200).json({ message: 'Record deleted successfully' });
  } catch (error) {
    console.error('Error deleting goodsOut record:', error);
