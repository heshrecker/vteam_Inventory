...
const GoodsOut = ({ loggedInUser }) => {
  ...
  return (
    <div>
      ...
      {/* Receipt Preview */}
      <h3 style={{ marginTop: 40 }}>Receipt Preview</h3>
      <div
        ref={receiptRef}
        style={{
          border: '1px solid #ccc',
      ...
      >
        <h2 style={{ textAlign: 'center' }}>Goods Out Receipt</h2>
        <p><strong>Date:</strong> {new Date().toLocaleString()}</p>
        <p><strong>Receiver:</strong> {selectedReceiver || '-'}</p>
        <p><strong>Printed By:</strong> {loggedInUser || '-'}</p>

        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ borderBottom: '1px solid #000', padding: '8px 4px' }}>Item Name</th>
              <th style={{ borderBottom: '1px solid #000', padding: '8px 4px' }}>Quantity</th>
            </tr>
          </thead>
          <tbody>
            {addedItems.length === 0 ? (
              <tr><td colSpan={2} style={{ textAlign: 'center', padding: 8 }}>No items added</td></tr>
            ) : (
              addedItems.map(item => (
                <tr key={item.id}>
                  <td style={{ borderBottom: '1px solid #ccc', padding: '8px 4px' }}>{item.name}</td>
                  <td style={{ borderBottom: '1px solid #ccc', padding: '8px 4px', textAlign: 'center' }}>{item.qty}</td>
                </tr>
              ))
            )}
          </tbody>
        ...
        <p style={{ marginTop: 8, textAlign: 'right', fontWeight: 'bold' }}>
          Received By:
        </p>
      </div>
    </div>
